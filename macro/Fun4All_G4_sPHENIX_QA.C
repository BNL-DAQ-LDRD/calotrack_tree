...
int Fun4All_G4_sPHENIX_QA(
    const int nEvents = 1,
    const string &inputFile =
        "/sphenix/sim/sim01/sphnxpro/MDC1/sHijing_HepMC/data/sHijing_0_20fm-0000000001-00000.dat",
    const string &outputFile = "G4sPHENIX.root",
    const string &embed_input_file =
        "https://www.phenix.bnl.gov/WWW/publish/phnxbld/sPHENIX/files/"
        "sPHENIX_G4Hits_sHijing_9-11fm_00000_00010.root",
    const int skip = 1, const string &outdir = ".") {
  Fun4AllServer *se = Fun4AllServer::instance();
  se->Verbosity(2);

  // Opt to print all random seed used for debugging reproducibility. Comment
  // out to reduce stdout prints.
  ...
  // Either:
  // read previously generated g4-hits files, in this case it opens a DST and
  // skips the simulations step completely. The G4Setup macro is only loaded to
  // get information about the number of layers used for the cell reco code
  //  Input::READHITS = true;
  ...
  // And
  // Further choose to embed newly simulated events to a previous simulation.
  // Not compatible with `readhits = true` In case embedding into a production
  // output, please double check your G4Setup_sPHENIX.C and G4_*.C consistent
  // with those in the production macro folder E.g.
  ...
  // Input::BEAM_CONFIGURATION = Input::AA_COLLISION; // Input::AA_COLLISION
  // (default), Input::pA_COLLISION, Input::pp_COLLISION
  ...
  //  Input::BEAM_CONFIGURATION = Input::AA_COLLISION; // for 2023 sims we want
  //  the AA geometry for no pileup sims Input::BEAM_CONFIGURATION =
  //  Input::pp_COLLISION; // for 2024 sims we want the pp geometry for no
  //  pileup sims Input::BEAM_CONFIGURATION = Input::pA_COLLISION; // for pAu
  //  sims we want the pA geometry for no pileup sims
  ...
  if (Input::SIMPLE) {
    ...
    if (Input::HEPMC || Input::EMBED) {
      ...
      INPUTGENERATOR::SimpleEventGenerator[0]
          ->set_existing_vertex_offset_vector(0.0, 0.0, 0.0);
    } else {
      INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_function(
          PHG4SimpleEventGenerator::Gaus, PHG4SimpleEventGenerator::Gaus,
          PHG4SimpleEventGenerator::Gaus);
      INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_mean(
          0., 0., 0.);
      INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_width(
          0.01, 0.01, 5.);
    }
    ...
    INPUTGENERATOR::SimpleEventGenerator[1]->set_existing_vertex_offset_vector(
        0.0, 0.0, 0.0);
  ...
  if (Input::UPSILON) {
    ...
    if (Input::HEPMC || Input::EMBED) {
      ...
      INPUTGENERATOR::VectorMesonGenerator[0]
          ->set_existing_vertex_offset_vector(0.0, 0.0, 0.0);
    }
  }
  // particle gun
  ...
  if (Input::HEPMC) {
    ...
    // INPUTMANAGER::HepMCInputManager->set_vertex_distribution_width(100e-4,
    // 100e-4, 8, 0);  //optional collision smear in space, time
    //    INPUTMANAGER::HepMCInputManager->set_vertex_distribution_mean(0,0,0,0);//optional
    //    collision central position shift in space, time
    // //optional choice of vertex distribution function in space, time
    // INPUTMANAGER::HepMCInputManager->set_vertex_distribution_function(PHHepMCGenHelper::Gaus,
    // PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus);
    //! embedding ID for the event
    //! positive ID is the embedded event of interest, e.g. jetty event from
    //! pythia negative IDs are backgrounds, .e.g out of time pile up collisions
    ...
    if (Input::PILEUPRATE > 0) {
      ...
      INPUTMANAGER::HepMCPileupInputManager->CopyHelperSettings(
          INPUTMANAGER::HepMCInputManager);
  ...
  // Option to convert DST to human command readable TTree for quick poke around
  // the outputs
  ...
  Enable::MBDFAKE = true; // Smeared vtx and t0, use if you don't want real
                          // MBD/BBC in simulation
  ...
  Enable::INTT = true;
  //  Enable::INTT_ABSORBER = true; // enables layerwise support structure
  //  readout Enable::INTT_SUPPORT = true; // enable global support structure
  //  readout
  ...
  Enable::GLOBAL_RECO =
      (Enable::MBDFAKE || Enable::MBDRECO || Enable::TRACKING_TRACK) && true;
  Enable::TRACKING_EVAL =
      Enable::TRACKING_TRACK && Enable::GLOBAL_RECO && false;
  ...
  // set flags to simulate and correct TPC distortions, specify distortion and
  // correction files
  // G4TPC::ENABLE_STATIC_DISTORTIONS = true;
  // G4TPC::static_distortion_filename =
  // std::string("/sphenix/user/rcorliss/distortion_maps/2023.02/Summary_hist_mdc2_UseFieldMaps_AA_event_0_bX180961051_0.distortion_map.hist.root");
  // G4TPC::ENABLE_STATIC_CORRECTIONS = true;
  // G4TPC::static_correction_filename =
  // std::string("/sphenix/user/rcorliss/distortion_maps/2023.02/Summary_hist_mdc2_UseFieldMaps_AA_smoothed_average.correction_map.hist.root");
  ...
  Enable::HCALOUT_EVAL =
      Enable::HCALOUT_G4Hit && Enable::HCALOUT_CLUSTER && true;
  ...
  //  Enable::BEAMLINE_ABSORBER = true;  // makes the beam line magnets
  //  sensitive volumes Enable::BEAMLINE_BLACKHOLE = true; // turns the beamline
  //  magnets into black holes
  Enable::ZDC = true;
  ...
  Enable::CALOTRIGGER = Enable::CEMC_TOWER && Enable::HCALIN_TOWER &&
  ...
  Enable::HIJETS = Enable::JETS && Enable::CEMC_TOWER && Enable::HCALIN_TOWER &&
                   Enable::HCALOUT_TOWER && false;

  // 3-D topoCluster reconstruction, potentially in all calorimeter layers
  Enable::TOPOCLUSTER = Enable::CEMC_TOWER && Enable::HCALIN_TOWER &&
                        Enable::HCALOUT_TOWER && false;
  ...
  // Enable::BLACKHOLE_FORWARD_SAVEHITS = false; // disable forward/backward
  // hits BlackHoleGeometry::visible = true;
  ...
  //  G4WORLD::WorldMaterial = "G4_AIR"; // set to G4_GALACTIC for material
  //  scans
  ...
  //  string("/Field/Map/sphenix3dbigmapxyz.root");  // default map from the
  //  calibration database G4MAGNET::magfield = "1.5"; // alternatively to
  //  specify a constant magnetic field, give a float number, which will be
  //  translated to solenoidal field in T, if string use as fieldmap name
  //  (including path)
  //  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar
  //  field strength of 1.4T
  ...
  // if enabled, do topoClustering early, upstream of any possible jet
  // reconstruction
  ...
  if (Enable::TPC_CLUSTER) {
    if (G4TPC::ENABLE_DIRECT_LASER_HITS ||
        G4TPC::ENABLE_CENTRAL_MEMBRANE_HITS) {
      TPC_LaserClustering();
    } else {
      TPC_Clustering();
    }
  ...
  */

  {
    ...
    double fieldstrength =
        std::numeric_limits<double>::quiet_NaN(); // set by isConstantField if
                                                  // constant
    bool ConstField =
        isConstantField(G4MAGNET::magfield_tracking, fieldstrength);
    if (ConstField) {
      ...
      seeder->magFieldFile(
          G4MAGNET::magfield_tracking); // to get charge sign right
    }
  ...
  if (Enable::GLOBAL_RECO && Enable::GLOBAL_FASTSIM) {
    cout << "You can only enable Enable::GLOBAL_RECO or "
            "Enable::GLOBAL_FASTSIM, not both"
         << endl;
    gSystem->Exit(1);
  }
  ...
  if (Enable::TRACKING_EVAL)
    Tracking_Eval(outputroot + "_g4svtx_eval.root");
  ...
  // the ntuple file is for diagnostics, it is produced only if the flag is set
  // in G4_Tracking.C
  ...
  if (Enable::KFPARTICLE && Input::DZERO)
    KFParticle_D0_Reco();
  //makes module happy
  rc->set_StringFlag("Sim_File_Name", "bla");
  calotrkana *caloana24 = new calotrkana("calotrkana", "testout.root");
  //caloana24->set_npart_range(0, 400);
  caloana24->set_HI(false);
  caloana24->set_tracking_only();
  ...
  if (Enable::TRACKING_QA && Enable::CEMC_QA && Enable::HCALIN_QA &&
      Enable::HCALOUT_QA)
    QA_G4CaloTracking();
  ...
  if (Enable::DSTOUT) {
    ...
    Fun4AllDstOutputManager *out =
        new Fun4AllDstOutputManager("DSTOUT", FullOutFile);
  ...
  if (nEvents == 0 && !Input::HEPMC && !Input::READHITS && INPUTEMBED::REPEAT) {
    cout << "using 0 for number of events is a bad idea when using particle "
            "generators"
         << endl;
